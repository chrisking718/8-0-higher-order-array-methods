...
function getSongsBySaib(songs) {
  ...
  let saib = songs.filter((contain) =>{
    return contain.artist === "Saib"
  })
 return saib
}
...
function getSongsOverThreeMinutes(songs) {
  let time = songs.filter((length) =>{
    return length.runtimeInSeconds > 180
  })
  return time
}
...
function getTitleTracks(songs) {
  let same = songs.filter((copy) => {
    return copy.title === copy.album
  })
  return same
}
